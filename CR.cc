...
namespace CR
{
  ...
  private:
    ...
    const unsigned int                  chemical_degree;
    FE_Q<dim>                           chemical_fe;
    DoFHandler<dim>                     chemical_dof_handler;
    ConstraintMatrix                    chemical_constraints;

    TrilinosWrappers::SparseMatrix      chemical_mass_matrix;
    TrilinosWrappers::SparseMatrix      chemical_stiffness_matrix;
    TrilinosWrappers::SparseMatrix      chemical_matrix;

    TrilinosWrappers::MPI::Vector       chemical_solution;
    TrilinosWrappers::MPI::Vector       old_chemical_solution;
    TrilinosWrappers::MPI::Vector       old_old_chemical_solution;
    TrilinosWrappers::MPI::Vector       chemical_rhs;
    ...
    bool                                rebuild_chemical_matrices;
  ...
  BoussinesqFlowProblem<dim>::BoussinesqFlowProblem ()
    ...
    rebuild_chemical_matrices (true),
  ...
  void BoussinesqFlowProblem<dim>::setup_dofs ()
  {
    ...
    const unsigned int n_u = stokes_dofs_per_block[0],
                       ...
                       n_C = chemical_dof_handler.n_dofs();
...

    {
      ...
      {
      chemical_matrix.clear ();

      DynamicSparsityPattern dspc (n_C, n_C);
      DoFTools::make_sparsity_pattern (chemical_dof_handler, dspc,
                                       chemical_constraints, false);

      chemical_matrix.reinit (dspc);
    }

    IndexSet temperature_partitioning = complete_index_set (n_T);
    IndexSet chemical_partitioning = complete_index_set (n_C);
    ...
    chemical_solution.reinit (chemical_partitioning, MPI_COMM_WORLD);
    old_chemical_solution.reinit (chemical_partitioning, MPI_COMM_WORLD);
    old_old_chemical_solution.reinit (chemical_partitioning, MPI_COMM_WORLD);

    chemical_rhs.reinit (chemical_partitioning, MPI_COMM_WORLD);
...
//
 template <int dim>
  void BoussinesqFlowProblem<dim>::assemble_chemical_matrix ()
  {
     if (rebuild_chemical_matrices == false)
       return;

    chemical_matrix = 0;

    QGauss<dim>   quadrature_formula (chemical_degree+2);
    FEValues<dim> chemical_fe_values (chemical_fe, quadrature_formula,
                                         update_values    | update_gradients |
                                         update_JxW_values);
    ...
    std::vector<double>         phi_C       (dofs_per_cell);
    std::vector<Tensor<1,dim> > grad_phi_C  (dofs_per_cell);
    ...
    for (; cell!=endc; ++cell)
      {
        ...
        for (unsigned int q=0; q<n_q_points; ++q)
          {
            for (unsigned int k=0; k<dofs_per_cell; ++k)
              {
                grad_phi_C[k] = chemical_fe_values.shape_grad (k,q);
                phi_C[k]      = chemical_fe_values.shape_value (k, q);
              }
            ...
            for (unsigned int i=0; i<dofs_per_cell; ++i)
              for (unsigned int j=0; j<dofs_per_cell; ++j)
                {
                  local_matrix(i,j)
                  += ((phi_C[i] * phi_C[j]) + (time_step * EquationData::kappa * grad_phi_C[i] * grad_phi_C[j] )
                      *
        ...
        chemical_constraints.distribute_local_to_global (local_matrix,
                                                            local_dof_indices,
                                                            chemical_matrix);

      }

    rebuild_chemical_matrices = false;
  }


  template <int dim>
  void BoussinesqFlowProblem<dim>::
  assemble_chemical_system (const double maximal_velocity)
  {
    // const bool use_bdf2_scheme = (timestep_number != 0);


        // chemical_matrix.copy_from (chemical_matrix);
        // temperature_matrix.add (time_step, temperature_stiffness_matrix);

    chemical_rhs = 0;

    const QGauss<dim> quadrature_formula(chemical_degree+2);
    FEValues<dim>     chemical_fe_values (chemical_fe, quadrature_formula,
                                             update_values    |
                                             update_gradients |
                                             update_hessians  |
    ...
    for (; cell!=endc; ++cell, ++stokes_cell)
      {
        ...
        chemical_fe_values.get_function_values (old_chemical_solution,
                                                   old_chemical_values);
        chemical_fe_values.get_function_values (old_old_chemical_solution,
                                                   old_old_chemical_values);

        chemical_fe_values.get_function_gradients (old_chemical_solution,
                                                      old_chemical_grads);
        chemical_fe_values.get_function_gradients (old_old_chemical_solution,
                                                      old_old_chemical_grads);

        chemical_fe_values.get_function_laplacians (old_chemical_solution,
                                                       old_chemical_laplacians);
        chemical_fe_values.get_function_laplacians (old_old_chemical_solution,
                                                       old_old_chemical_laplacians);

        temperature_right_hand_side.value_list (chemical_fe_values.get_quadrature_points(),
                                                gamma_values);
        ...
        for (unsigned int q=0; q<n_q_points; ++q)
          {
            ...
            for (unsigned int i=0; i<dofs_per_cell; ++i)
              local_rhs(i) += (old_chemical_values[q] * phi_C[i]
                               -
                               time_step *
                               old_velocity_values[q] *  old_chemical_grads[q]* phi_C[i]
                              //  -
                              //  time_step *
                              //  nu * ext_grad_T * grad_phi_T[i]
                               +
                               time_step *
                               gamma_values[q] * phi_C[i])
                              *
                              chemical_fe_values.JxW(q);
          }

        cell->get_dof_indices (local_dof_indices);
        chemical_constraints.distribute_local_to_global (local_rhs,
                                                            local_dof_indices,
                                                            chemical_rhs);
      }
  }
...
//end chemical
  ...
  void BoussinesqFlowProblem<dim>::solve ()
  {
    ...
    chemical_solution = old_chemical_solution;

    assemble_chemical_system (maximal_velocity);
    {

      SolverControl solver_control_c (chemical_matrix.m(),
                                    1e-8*chemical_rhs.l2_norm());
      SolverCG<TrilinosWrappers::MPI::Vector> cgc (solver_control_c);

      TrilinosWrappers::PreconditionIC preconditioner_c;
      preconditioner_c.initialize (chemical_matrix);

      cgc.solve (chemical_matrix, chemical_solution,
                chemical_rhs, preconditioner_c);

      chemical_constraints.distribute (chemical_solution);

      std::cout << "   "
                << solver_control_c.last_step()
                << " CG iterations for chemical."
                << std::endl;


      double min_chemical = chemical_solution(0),
             max_chemical = chemical_solution(0);
      for (unsigned int i=0; i<chemical_solution.size(); ++i)
        {
  ...
  void BoussinesqFlowProblem<dim>::output_results ()  const
  {
    ...
    data_out.add_data_vector (temperature_dof_handler, temperature_solution,
                              "T");
    data_out.add_data_vector (chemical_dof_handler, chemical_solution,
                              "C");
    data_out.build_patches (std::min(stokes_degree, temperature_degree));

    std::ostringstream filename;
    filename << "output/solution-" << Utilities::int_to_string(timestep_number, 4) << ".vtk";
  ...
  void BoussinesqFlowProblem<dim>::refine_mesh (const unsigned int max_grid_level)
  {
    ...
    std::vector<TrilinosWrappers::MPI::Vector> x_chemical (2);
    x_chemical[0] = chemical_solution;
    x_chemical[1] = old_chemical_solution;
    ...
    SolutionTransfer<dim,TrilinosWrappers::MPI::Vector>
    chemical_trans(chemical_dof_handler);

    triangulation.prepare_coarsening_and_refinement();
    temperature_trans.prepare_for_coarsening_and_refinement(x_temperature);
    stokes_trans.prepare_for_coarsening_and_refinement(x_stokes);
    chemical_trans.prepare_for_coarsening_and_refinement(x_chemical);
    ...
    std::vector<TrilinosWrappers::MPI::Vector> tmp_chemical (2);
    tmp_chemical[0].reinit (chemical_solution);
    tmp_chemical[1].reinit (chemical_solution);
    chemical_trans.interpolate(x_chemical, tmp_chemical);

    chemical_solution = tmp_chemical[0];
    old_chemical_solution = tmp_chemical[1];
    ...
    chemical_constraints.distribute(chemical_solution);
    chemical_constraints.distribute(old_chemical_solution);
...
start_time_iteration:
    ...
    VectorTools::project (chemical_dof_handler,
                          chemical_constraints,
                          QGauss<dim>(chemical_degree+2),
                          EquationData::TemperatureInitialValues<dim>(),
                          old_chemical_solution);
    ...
    do
      {
        ...
        old_old_chemical_solution = old_chemical_solution;
        old_chemical_solution     = chemical_solution;
